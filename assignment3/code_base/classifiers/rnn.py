...
class SentimentAnalysisRNN(object):
    ...
    def loss(self, wordvecs, labels, mask):
        ...
        # Forward
        h_rnn, cache_rnn = rnn_forward(wordvecs, h0, Wx, Wh, b)

        scores_a, cache_a = temporal_affine_forward(h_rnn, W_a, b_a)

        scores_av, cache_av = average_forward(scores_a, mask)

        scores_fc, cache_fc = affine_forward(scores_av, W_fc, b_fc)

        loss, dscores = softmax_loss(scores_fc, labels)
        
        #backward

        dh_fc, dW_fc, db_fc = affine_backward(dscores, cache_fc)

        dh_av = average_backward(dh_fc, cache_av)

        dh_a, dW_a, db_a = temporal_affine_backward(dh_av, cache_a)

        dx, dh0, dWx, dWh, db = rnn_backward(dh_a, cache_rnn)
